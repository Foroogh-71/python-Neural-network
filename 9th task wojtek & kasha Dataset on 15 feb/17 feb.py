# -*- coding: utf-8 -*-
"""
Created on Wed Feb 16 21:08:00 2022
...
"""
 ---- Loading data ----
"""
#file = pd.read_csv('NON_Gaussian_Simulation2.csv', skiprows=0)
file = pd.read_csv('Gaussian_Simulation.csv', skiprows=0)
...
# mergeFtrsR = np.vstack([ stdDD, meanDD, rmsDD,
#                         medianDD, maxDD])
...
y1 = model.fit_predict(pcaTrain[:, 1:3])
yhat1 = model.fit_predict(pcatest[:, 1:3])
...
# fit the model
y2=model.fit_predict(pcaTrain[:,1:3])
yhat2 = model.predict(pcatest[:, 1:3])
...
# fit model and predict clusters
yhat3 = model.fit_predict(pcaTrain[:, 1:3])
yhat3 = model.fit_predict(pcatest[:, 1:3])
...
y4=(model.fit_predict(pcaTrain[:, 1:3]) )
yhat4 =( model.predict(pcatest[:, 1:3]))
...
y5=(model.fit_predict(pcaTrain[:, 1:3]))
yhat5=(model.fit_predict(pcatest[:, 1:3]))
...
y6 = model.fit_predict(pcaTrain[:, 1:3])
# assign a cluster to each example
yhat6 = model.predict(pcatest[:, 1:3])
...
y7=model.fit_predict(pcaTrain[:, 1:3])
yhat7 = model.predict(pcatest[:, 1:3])
...
model = MeanShift()
y9=model.fit_predict(pcaTrain[:, 1:3])
yhat9 = model.predict(pcatest[:, 1:3])
...
# fit the model
trainresult = model.fit_predict(pcaTrain[:, 1:3])
# assign a cluster to each example
yhat10 = model.predict(pcatest[:, 1:3])

score = silhouette_score(pcatest[:, 1:3], yhat10)
...
#                  * first), matrix[i, 2:first+2], color(yhat10[i, 1]))
# for i in range(0, len(matrixtrain[:,1])):
#     axs[0][0].plot(range((int(matrixtrain[i, 0]))*first, int(matrixtrain[i, 0]+1)
#                  * first), matrixtrain[i, 2:first+2], color(trainresult[i, 1]))
axs[0][0].scatter(total[:,1] ,total[:,2])
axs[0,0].set_ylabel('pc1 amount')
axs[0,0].set_xlabel('pc2 amount')

axs6[1][0].set_ylabel('gaussian clustering')
axs[1,0].plot(total[:,1] )
axs[1,0].set_ylabel('pc1 amount')
...
axs[1,1].plot(t, signal)
axs[1,1].set_title('title= signal with noise')
axs[1,1].set_ylabel('amount')
axs[1,1].set_xlabel('number of datapoint')



plot4 = pyplot. figure('pca')

plt.scatter(total[:,1] ,total[:,2])
plt.xlabel("pc1")
plt.ylabel("pc2")
