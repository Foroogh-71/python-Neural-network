# -*- coding: utf-8 -*-
"""
Created on Mon Jan 31 22:15:18 2022
...
#-----------------------------------
pcatest=np.zeros((50,8));
pcatest[:, 0]=x_val[:, 0]
pcatest[:, 1:]=fit.fit_transform(x_val[:, 1:])
#------------------------------------------------------
matrix=np.zeros((50,11));
...
axs6[0].scatter(pcatest[:, 0]*20 ,xs,c=yhat10[:,1])
...
for i in range(0,50):
    ...
    axs6[1].plot(range((int(matrix[i,0]))*10,int(matrix[i,0]+1)*10),matrix[i,1:],color(yhat10[i,1]))
